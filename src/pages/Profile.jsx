...
export default function Profile() {
  ...
  return (
    <div style={{ padding: 20, maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, marginBottom: 16 }}>
          ...
          {!editing && (
            <button onClick={() => setEditing(true)} style={{
              ...
              border: '0.5px solid #ddd', background: 'transparent', color: '#666', cursor: 'pointer'
            }}>Edit profile</button>
          )}
        </div>

        {/* Stats */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, paddingTop: 14, borderTop: '0.5px solid #f0f0f0' }}>
          {[
            { label: 'Films scored', value: scores.length },
            { label: 'Avg score', value: avg },
            { label: 'Years active', value: new Date().getFullYear() - memberYear || '—' },
            { label: 'Top 10 highest', value: top10[0] ? parseFloat(top10[0].score).toFixed(1) : '—' },
          ].map(stat => (
            ...
            {top10.map((s, i) => (
              <div key={s.id} onClick={() => navigate(`/movie/${s.movie_id}`)} style={{ cursor: 'pointer' }}>
                <div style={{ position: 'relative', marginBottom: 6 }}>
                  {s.movies?.poster_url
                    ? <img src={s.movies.poster_url} alt={s.movies.title} style={{ width: '100%', height: 110, objectFit: 'cover', borderRadius: 8 }} />
                    : <div style={{ width: '100%', height: 110, borderRadius: 8, background: '#EEEDFE', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>🎬</div>
                  }
      ...
      {/* Privacy controls */}
      <div style={{ background: '#fff', borderRadius: 12, border: '0.5px solid #eee', padding: 16 }}>
        <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 12 }}>🔒 Privacy controls</div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div
                ...
                style={{
                  ...
                  background: privacy[item.key] ? '#534AB7' : '#ddd'
                }}
              >
                <div style={{
